...
const convertToJalali = (timestamp) => {
    ...
    return {
        jalali: `${jalaaliDate.jy}/${jalaaliDate.jm}/${jalaaliDate.jd}`, // تاریخ جلالی
        gregorian: gregorianDate // تاریخ میلادی
    };
};
...
app.get("/" + SUBSCRIPTION.split('/')[3] + "/:subId", async (req, res) => {
    try {
        ...
        const expiryDates = convertToJalali(trafficData.obj.expiryTime);
        const expiryTimeJalali = expiryDates.jalali; // تاریخ جلالی
        const expiryTimeGregorian = expiryDates.gregorian; // تاریخ میلادی
        ...
        const Backup_link = config.Backup_link1 + '\n' + config.Backup_link2 + '\n' + config.Backup_link3 + '\n' + config.Backup_link4 + '\n' + config.Backup_link5;
        ...
        if (userAgent && isBrowserRequest(userAgent)) {
            res.render("sub", {
                data: {
                    ...trafficData.obj,
                    expiryTimeJalali, // تاریخ جلالی
                    expiryTimeGregorian, // تاریخ میلادی
                    suburl,
            ...
            const combinedContent = decodedSuburlContent + '\n' + decodedBackupLink;
